...
async function run() {
    try {
        ...
        // sendRequest already alerted the user; stop further processing
        return;
    }
}
...
function sendRequest(url) {
    return fetch(url)
        .then((response) => {
            if (!response.ok) {
                // show code and status as requested
                alert(
        ...
        .catch((err) => {
            // network errors or thrown errors land here — alert if not already alerted
